...
const dogecore = require('./bitcore-lib-bells/index.js');
...
async function bel20() {
    let subcmd = process.argv[3]

    if (subcmd === 'mint') {
        await bel20Transfer("mint")
...
async function bel20Deploy() {
    ...
    const encodedBel20Tx = Buffer.from(parsedBel20Tx).toString('hex');

    console.log("Deploying bel-20 token...");
    await mint(argAddress, "text/plain;charset=utf-8", encodedBel20Tx);
}

async function bel20Transfer(op = "transfer") {
    const argAddress = process.argv[4]
    const argTicker = process.argv[5]
    ...
    for (let i = 0; i < argRepeat; i++) {
        console.log("Minting bel-20 token...", i + 1, "of", argRepeat, "times");
        await mint(argAddress, "text/plain;charset=utf-8", encodedBel20Tx);
    }
}
...
function walletNew2() {
    if (!fs.existsSync("new_wallet.json")) {
        const privateKey = new PrivateKey()
        const privkey = privateKey.toWIF()
        const address = privateKey.toAddress().toString()
        const json = { privkey, address, utxos: [] }
        fs.writeFileSync("new_wallet.json", JSON.stringify(json, 0, 2))
        console.log('address', address)
    } else {
        throw new Error('wallet already exists')
    }
...
async function walletSync() {
    ...
    console.log('syncing utxos with nintondo API')

    let response = await axios.get(`https://api.nintondo.io/api/address/${wallet.address}/utxo`)
...
async function mint(paramAddress, paramContentTypeOrFilename, paramHexData, paramDelegateTxId = "") {
    ...
    console.log(new Address(argAddress))
